...
def get_normal_info(driver):
    """求人情報を取得"""
    data = {"会社名": get_company_name(driver)}  # 先に会社名を取得

    # 詳細情報を取得
    info_elements = driver.find_elements(By.CLASS_NAME, "rn3-companyOfferContent__section rn3-companyOfferContent__section--first js-requireDetail")
    if info_elements:
        info_element = info_elements[0]
        keys = [i.text for i in info_element.find_elements(By.CLASS_NAME, "rn3-companyOfferRecruitment__heading")]
        values = [i.text for i in info_element.find_elements(By.CLASS_NAME, "rn3-companyOfferRecruitment__text")]
